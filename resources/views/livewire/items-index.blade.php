...
<div x-data="form()" class="py-12">
    ...
    <template x-teleport="#new">
        <x-primary-button x-on:click.prevent="openModal()">{{ __('New item') }}</x-primary-button>
    </template>
                ...
                <div x-data="{ showTranscationItems: null }" class="grid grid-cols-3 justify-start items-start gap-3">
                    @forelse ($this->items as $item)
                        <div class="flex flex-col items-center border rounded-lg p-3 shadow-md"
                            x-on:mouseenter="showTranscationItems = {{ $item->id }}"
                            x-on:mouseleave="showTranscationItems = null">
                            <div class="flex w-full items-center justify-between gap-3">
                                <div>{{ $item->name }}</div>
                                <div>{{ $item->unit }}</div>
                                <div>
                                    {{ $item->totalIncome - $item->totalOutgoing == 0 ? '' : $item->totalIncome - $item->totalOutgoing }}
                                </div>
                                <div class="border-s-2 ps-3 text-center">
                                    ...
                                    <button type="button"
                                        x-on:click.prevent="openModal({{ $item }})">Edit</button>
                                    <div class=" cursor-pointer select-none"
                                        x-on:click="showTranscationItems != {{ $item->id }} ? showTranscationItems = {{ $item->id }} : showTranscationItems = null">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke-width="1.5" stroke="currentColor" class="size-5">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            {{-- <div x-cloak x-show="showTranscationItems === {{ $item->id }}" x-transition
                                class="mt-3 w-full border-t-2 flex flex-col divide-y">

                                @forelse ($item->available_items as $item)
    ...
    <x-modal focusable dismissible maxWidth="lg" name="form-modal">
        <form x-on:submit.prevent="submit" class="p-6 flex flex-col gap-3">
            ...
            <div class="w-full">
                <label class="block" for="unit">unit</label>
                <x-select class="w-full" required x-model="form.unit">
                    <option value="">---</option>
